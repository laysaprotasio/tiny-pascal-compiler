...
class TinyPascalSemanticAnalyzer {
    ...
    analyzeProcedureDeclaration(node) {
        ...
        for (const param of node.params || []) {
            this.symbolTable.declareSymbol(param.name, {
    ...
    analyzeReturnStmt(node) {
        ...
        node.evaluatedValue = valInfo.value;
    ...
    analyzeStmt(stmt) {
        switch (stmt.type) {
            ...
            case 'BreakStmt':
                return this.analyzeBreakStmt(stmt);
            case 'ContinueStmt':
                return this.analyzeContinueStmt(stmt);
            case 'WriteStmt':
    ...
    analyzeAssign(node) {
        ...
        if (symbol.type !== valInfo.type) {
            throw new SemanticError(`Incompatibilidade de tipos na atribuição: '${symbol.type}' := '${valInfo.type}'`, node);
        }

        this.symbolTable.updateSymbol(target.name, {
            initialized: true,
            value: valInfo.value
        });
    ...
    analyzeBreakStmt(node) {
        if (this.loopDepth <= 0) {
            throw new SemanticError(`'break' usado fora de laço`, node);
        }
        node.inferredType = 'void';
    ...
    analyzeContinueStmt(node) {
        if (this.loopDepth <= 0) {
            throw new SemanticError(`'continue' usado fora de laco`, node);
        }
        node.inferredType = 'void';
        node.evaluatedValue = null;
        return node;
    ...
    analyzeExpr(expr) {
        switch (expr.type) {
            case 'Number':
                return { type: 'integer', value: expr.value };
            case 'Boolean':
                return { type: 'boolean', value: expr.value };
            case 'Identifier': {
                const symbol = this.symbolTable.lookupSymbol(expr.name);
                if (!symbol) throw new SemanticError(`Identificador '${expr.name}' não declarado`, expr);
                return { type: symbol.type, value: symbol.value };
            }
            case 'BinaryOp':
            ...
            case 'Not': {
                ...
                return { type: 'boolean', value: (info.value !== null ? !info.value : null) };
            }
            default:
